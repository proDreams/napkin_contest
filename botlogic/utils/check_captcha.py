...
from aiogram.types import FSInputFile, InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery, ChatMemberMember, \
    ChatMemberLeft, ChatMemberUpdated
import os
import random
...
def generate_captcha():
    ...
    logger.info(f"Каптча сгенерирована: {num1} + {num2} = {result}, сохранена в {img_path}")
...
async def check_user_captcha(bot: Bot, chat_id: int, user_id: int, text: str, message: types.Message):
        ...
        if users_data[user_id]['attempts'] >= 3:
            try:
                ...
                await bot.unban_chat_member(chat_id,
                                            user_id)  # Немедленное снятие бана, потому что нужно давать второй шанс
            except Exception as e:
                if "can't remove chat owner" in str(e):
